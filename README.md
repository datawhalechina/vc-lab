# 设计思维创造营 · 第一期（⚠️ Alpha内测版）

> ⚠️ Alpha内测版本警告：此为早期内部构建版本，尚不完整且可能存在错误，欢迎大家提Issue反馈问题或建议。

用14天，从生活洞察出发，亲手将模糊的"不爽"变成可视化产品概念。

## 项目受众

本课程面向对设计、硬件、AI或产品创新有好奇心的学习者，包括但不限于：
- 想要入门设计思维的学生
- 希望提升创新能力的产品经理
- 想要用设计思维解决问题的创业者

## 在线阅读

[https://Peter-303.github.io/VC-Lab](https://Peter-303.github.io/VC-Lab)

## 学习路线

| 阶段 | 时间 | 主题 | 核心产出 |
|------|------|------|----------|
| **第一阶段** | Day 1-2 | 建立设计视角 | 双钻模型思维练习 |
| **第二阶段** | Day 3-4 | 问题洞察 | 痛点地图、课题定义书 |
| **第三阶段** | Day 5-6 | 战略分析 | 竞品分析、POV陈述 |
| **中场休息** | Day 7 | 回顾与连接 | 复盘与交流 |
| **第四阶段** | Day 8-9 | 概念定义 | 产品设计蓝图、灵感板 |
| **第五阶段** | Day 10-11 | 视觉创造 | AI概念图、设计说明 |
| **最终产出** | Day 12 | 整合输出 | 概念故事板 |
| **结营展示** | Day 13 | 成果分享 | 线上故事会 |

## 目录

| 章节名 | 简介 | 状态 |
|---|---|---|
| [第1章 认知唤醒与问题洞察](https://github.com/Peter-303/VC-Lab/blob/main/docs/chapter1) | 建立设计视角，学习双钻模型，绘制痛点地图 | ✅ |
| [第2章 战略分析与概念定义](https://github.com/Peter-303/VC-Lab/blob/main/docs/chapter2) | 竞品分析，定义POV，绘制产品蓝图 | ✅ |
| [第3章 视觉创造与成果展示](https://github.com/Peter-303/VC-Lab/blob/main/docs/chapter3) | AI视觉生成，概念选择，制作故事板 | ✅ |

## 贡献者名单

| 姓名 | 职责 | 简介 |
|---|---|---|
| 龙若函 | 项目负责人 | 框架搭建、全文写作与校对 |
|  韩涵  | 运营负责人 | 全文优化、习题设计、课程校对与平台运营 |
| 尚启峰 | 联合发起者 | 第一章课程校对、协同优化 |

## 参与贡献

- 如果你对本课程有建议、发现问题，或希望参与后续内容完善，请通过以下方式联系我们：

- 提交Issue：若发现了问题，可以在GitHub仓库中提交问题反馈。
- 提交Pull Request：若想参与贡献本项目，可以在GitHub仓库中拉取请求。
- 如果提交后无人回复，请联系[保姆团队](https://github.com/datawhalechina/DOPMC/blob/main/OP.md)的同学进行反馈跟进~

- 如果你对 Datawhale 很感兴趣并想要发起一个新的项目，请按照[Datawhale开源项目指南](https://github.com/datawhalechina/DOPMC/blob/main/GUIDE.md)进行操作即可~

- 我们期待与您一起，让设计课程越来越好。

## 关注我们

<div align=center>
<p>扫描下方二维码关注公众号：Datawhale</p>
<img src="https://raw.githubusercontent.com/datawhalechina/pumpkin-book/master/res/qrcode.jpeg" width = "180" height = "180">
</div>

## LICENSE

本作品采用[知识共享署名-非商业性使用-相同方式共享 4.0 国际许可协议](http://creativecommons.org/licenses/by-nc-sa/4.0/)进行许可。
